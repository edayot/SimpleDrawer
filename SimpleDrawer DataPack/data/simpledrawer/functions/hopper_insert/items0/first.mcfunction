...
data modify storage simpledrawer:temp ItemHopper set from block ~ ~2 ~ Items[{Slot:0b}]
...
execute store result score nbhopper simpledrawer.tempC run data get block ~ ~2 ~ Items[{Slot:0b}].Count
scoreboard players remove nbhopper simpledrawer.tempC 1
execute store result block ~ ~2 ~ Items[{Slot:0b}].Count byte 1 run scoreboard players get nbhopper simpledrawer.tempC
