...
#execute positioned ~ ~-1 ~ if data block ~ ~ ~ {Lock:"SD_DrawerPlacer"} run function simpledrawerplace/place

setblock 0 0 0 chest{Items:[{Slot:0b,id:"minecraft:diamond",Count:1b}]} replace
item modify block 0 0 0 container.0 simpledrawerget_pos

execute store result score delta_x SD_tempC run data get block 0 0 0 Items[0].tag.pos[0]

execute store result score delta_y SD_tempC run data get block 0 0 0 Items[0].tag.pos[1]

execute store result score delta_z SD_tempC run data get block 0 0 0 Items[0].tag.pos[2]
...
summon marker 0 0 0 {Tags:["SD_Placer"]}
...
execute at @e[type=marker,tag=SD_Placer] run function simpledrawerplace/place
