# @public
# loot give @s loot simpledrawer:impl/items/guide
# loot give @s loot smithed.crafter:blocks/table

loot give @s loot simpledrawer:impl/items/new_drawer
loot give @s loot simpledrawer:impl/items/double_new_drawer
loot give @s loot simpledrawer:impl/items/quadruple_new_drawer
loot give @s loot simpledrawer:impl/items/compacting_new_drawer

loot give @s loot simpledrawer:impl/items/hopper_upgrade
loot give @s loot simpledrawer:impl/items/iron_upgrade
loot give @s loot simpledrawer:impl/items/gold_upgrade
loot give @s loot simpledrawer:impl/items/diamond_upgrade
loot give @s loot simpledrawer:impl/items/star_upgrade
loot give @s loot simpledrawer:impl/items/netherite_upgrade
loot give @s loot simpledrawer:impl/items/downgrade_wrench
