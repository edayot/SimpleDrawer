...
data modify storage simpledrawer:io output set from entity @s item.components."minecraft:container"[{slot:15}].item
...
execute store result entity @s item.components."minecraft:container"[{slot:15}].item.count int 1 run scoreboard players get #newCount simpledrawer.math
