...
execute if score #itemio.major load.status matches 0 if score #itemio.minor load.status matches 5 if score #itemio.patch load.status matches 0.. run scoreboard players set #simpledrawer.itemio load.status 1
...
execute unless score #simpledrawer.itemio load.status matches 1 run tellraw @a [{"translate":"simpledrawer.tellraw_prefix","color":"dark_red"},{"text":"Error Loading SimpleDrawer, ItemIO v0.5.0+ is required.","color":"red"}]
