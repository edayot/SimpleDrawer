...
execute if score #itemio.major load.status matches 0 if score #itemio.minor load.status matches 2 if score #itemio.patch load.status matches 0.. run scoreboard players set #simpledrawer.itemio load.status 1
...
execute if score #smithed.actionbar.major load.status matches 0 if score #smithed.actionbar.minor load.status matches 1 if score #smithed.actionbar.patch load.status matches 0 run scoreboard players set #simpledrawer.smithed.actionbar load.status 1
...
execute unless score #simpledrawer.itemio load.status matches 1 run tellraw @a [{"translate":"simpledrawer.tellraw_prefix","color":"dark_red"},{"text":"Error Loading SimpleDrawer, ItemIO v0.2.0+ is required.","color":"red"}]
