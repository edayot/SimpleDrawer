...
execute store result score nbhopper SD_tempC run data get block ~ ~2 ~ Items[{Slot:3b}].Count
...
execute if score Count SD_tempC <= maxCount SD_tempC store result block ~ ~2 ~ Items[{Slot:3b}].Count byte 1 run scoreboard players get nbhopper SD_tempC
execute if score Count SD_tempC <= maxCount SD_tempC as @e[type=glow_item_frame,tag=SD_selected,limit=1] run function drawer:working/display
